...
INTERFACE=$(ip -o -f inet addr show | awk '{ print $4 }')
NETWORK_CARD=$(lspci | grep -i ethernet | awk -F: '{print $2}' | xargs)

#getting system status information
USERS_LOGGED=$(who | awk '{ print $1 }' | sort | uniq | xargs)
DISK_SPACE=$(df -h --output=target,size | grep '^/' | xargs)
PROCESS_COUNT=$(ps aux | wc -l)
LOAD_AVERAGES=$(cat /proc/loadavg | awk '{ print $1, $2, $3}')
MEM_ALLOCATED=$(free -h)
LIST_PORTS=$(ss -ltn | awk '{ print $4 }' | cut -d: -f2 | xargs)
UFW_RULES=$(sudo ufw status | tail -n +3)
#cat output report
cat <<EOF
